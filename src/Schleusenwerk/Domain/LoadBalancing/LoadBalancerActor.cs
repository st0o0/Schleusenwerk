...
public sealed class LoadBalancerActor : ReceiveActor
{
    ...
    public static Props CreateProps(IReadOnlyList<UpstreamTarget> upstreams) =>
        Props.Create(() => new LoadBalancerActor(upstreams));
        ...
        foreach (var upstream in healthyUpstreams)
        {
            for (var i = 0; i < upstream.Weight; i++)
            {
                var child = Context.ActorOf(
                    UpstreamRouteeActor.CreateProps(upstream),
                    $"routee-g{gen}-{index++}");
